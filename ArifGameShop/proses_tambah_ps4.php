  ...
  $nama_gambar_baru = $angka_acak.'-'.$gambar_game; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
                move_uploaded_file($file_tmp, 'assets/imgs/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO ps4 (nama_game, deskripsi, kategori, harga, gambar_game) VALUES ('$nama_game', '$deskripsi', '$kategori', '$harga', '$nama_gambar_baru')";
                  ...
                  if(!$result){
                    ...
                    echo "<script>alert('Data berhasil ditambah.');window.location='halaman_ps4.php';</script>";
                  }
   ...
   $query = "INSERT INTO ps4 (nama_game, deskripsi, kategori, harga, gambar_game) VALUES ('$nama_game', '$deskripsi', '$kategori', '$harga', null)";
                  $result = mysqli_query($koneksi, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($koneksi).
